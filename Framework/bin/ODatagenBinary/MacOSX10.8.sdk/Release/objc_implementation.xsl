...
<xsl:for-each select="schema_1_0:End | schema_1_1:End | schema_1_2:End">
		[arr addObject:[[[NSDictionary alloc]initWithObjectsAndKeys:@"<xsl:value-of select="@Role"/>",@"EndRole",@"<xsl:value-of select="substring-after(@Type,concat($service_namespace,'.'))"/>",@"Type",@"<xsl:value-of select="@Multiplicity"/>",@"Multiplicity",nil] autorelease]];
...
</xsl:otherwise></xsl:choose><xsl:if test="schema_1_0:Parameter | schema_1_1:Parameter | schema_1_2:Parameter">	
...
{
	<xsl:for-each select="schema_1_0:Property | schema_1_1:Property | schema_1_2:Property">
	<xsl:if test="@Type = 'Edm.String'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.Binary'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.Boolean'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.DateTime'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.Guid'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.Int16'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.Int32'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.Int64'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.Decimal'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.Single'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	<xsl:if test="@Type = 'Edm.Double'">
	m_<xsl:value-of select="@Name"/> = nil;
	</xsl:if>
	</xsl:for-each>
	<xsl:for-each select="schema_1_0:NavigationProperty | schema_1_1:NavigationProperty | schema_1_2:NavigationProperty">
